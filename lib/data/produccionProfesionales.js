...
export async function obtenerProduccionPorProfesional(fechaInicio, fechaFin) {
  const [
    ...
    supabase
      .from("caja_general")
      .select(
        "fecha, pago, prestaciones, precio_anterior, profesional_atencion_id, paciente_id, id_documento, tipo_documento, paciente:pacientes(apellido_y_nombre)"
      )
  ...
  // Dentro de una cuota que mezcla especialidades, ¿quién atendió realmente
  // cada parte ese día? Se cruza con el turno de esa paciente en esa misma
  // fecha: si el turno tiene una prestación que matchea (por nombre) con
  // una especialidad del catálogo, esa especialidad se le acredita a quien
  // figura como profesional de turno ese día — no a quien cobró la cuota,
  // que puede ser otra persona (ej. la responsable del plan, que solo cobró
  // pero no hizo esa parte puntual). Si no hay turno ese día para esa
  // especialidad, se sigue usando quien cobró (como antes).
  const especialidadPorNombrePrestacion = {};
  const turnosPorPacienteFecha = {};
  if (numerosPlanAUsar.length > 0) {
    const [{ data: planes }, { data: catalogoCompleto }, { data: turnosDelPeriodo }] = await Promise.all([
      supabase.from("planes_financiacion").select("numero_plan, presupuesto_id").in("numero_plan", numerosPlanAUsar),
      supabase.from("catalogo_prestaciones").select("id, especialidad, prestacion"),
      supabase
        .from("turnos_general")
    ...
    for (const c of catalogoCompleto || []) {
      especialidadPorCatalogoId[c.id] = c.especialidad || "Sin especialidad";
      if (c.especialidad) especialidadPorNombrePrestacion[normalizarTexto(c.prestacion)] = c.especialidad;
    }

    for (const t of turnosDelPeriodo || []) {
      if (!t.profesional_de_turno_id) continue;
      const clave = `${t.paciente_id}|${t.fecha}`;
      if (!turnosPorPacienteFecha[clave]) turnosPorPacienteFecha[clave] = [];
      turnosPorPacienteFecha[clave].push(t);
    }
  ...
  // Para una cuota puntual (paciente + fecha), arma especialidad -> quién
  // atendió ese día según el turno. Si el turno tiene varias prestaciones de
  // la misma especialidad con profesionales distintos, se queda con el
  // primero (caso raro y ambiguo, no vale la pena inventar un desempate).
  function profesionalPorEspecialidadEseDia(pacienteId, fecha) {
    const turnos = turnosPorPacienteFecha[`${pacienteId}|${fecha}`] || [];
    const mapa = {};
    for (const t of turnos) {
      for (const p of t.prestaciones || []) {
        const esp = especialidadPorNombrePrestacion[normalizarTexto(p.prestacion)];
        if (esp && !mapa[esp]) mapa[esp] = t.profesional_de_turno_id;
      }
    }
    return mapa;
  }
  ...
  for (const c of cobrosGeneral || []) {
    ...
    if (prestaciones.length > 0 && !c.precio_anterior) {
      ...
      // proporcional al peso de cada una en el total del presupuesto. Cada
      // parte se le acredita a quien realmente la atendió ese día (según el
      // turno), y solo si no hay turno que lo diga se usa quien cobró la
      // cuota — así no se le acredita a una profesional una especialidad
      // que hizo otra persona el mismo plan.
      const paciente = c.paciente?.apellido_y_nombre ?? "—";
      const profesionalPorEsp = profesionalPorEspecialidadEseDia(c.paciente_id, c.fecha);
      for (const { especialidad, share } of especialidadSharesPorPlan[c.id_documento]) {
        const monto = Number(c.pago) * share;
        const profesionalRealId = (especialidad && profesionalPorEsp[especialidad]) || c.profesional_atencion_id || "sin-asignar";
        const entradaEspecialidad = obtenerEntrada(profesionalRealId);
