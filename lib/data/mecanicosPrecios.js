...
// Nombres de trabajo que ESE mecánico usa (de la comparativa de precios),
// para sugerir mientras se escribe el "Tipo de trabajo" — así se carga con
// el nombre que el mecánico reconoce, no con el nombre de nuestro catálogo.
export async function obtenerTrabajosMecanico(laboratorio) {
  if (!laboratorio?.trim()) return [];
  const { data, error } = await supabase.rpc("obtener_trabajos_mecanico", { p_laboratorio: laboratorio.trim() });
  if (error) throw error;
  return data.map((f) => f.trabajo);
}
