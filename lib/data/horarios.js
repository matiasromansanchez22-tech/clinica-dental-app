...
// El registro del usuario logueado en una fecha puntual (para el widget
// de marcar entrada/salida, o para cargar/corregir un día anterior) —
// null si todavía no marcó nada ese día.
export async function obtenerRegistroDeFecha(fecha) {
  const {
    data: { user },
...
// Carga o corrige a mano el propio horario de un día (típicamente uno
// anterior a hoy) — crea el registro si no existía, o lo actualiza si ya
// estaba.
export async function guardarRegistroPropio({ id, fecha, horaEntrada, horaSalida }) {
  if (id) {
    return actualizarRegistro(id, { horaEntrada, horaSalida });
  }
  const {
    data: { user },
  } = await supabase.auth.getUser();
  return crearRegistroManual({ usuarioId: user.id, fecha, horaEntrada, horaSalida });
}
...
// El input de hora del navegador a veces "completa solo" en 00:00 si se
// lo toca sin querer (ej. con las flechitas) sin haber tipeado nada. Nadie
// marca entrada/salida real a medianoche en punto, así que ese valor se
// trata como si el campo hubiera quedado vacío, en vez de guardarlo como
// si fuera una hora real (lo que después bloqueaba marcar salida de
// verdad, porque el sistema ya creía que estaba cargada).
...
export async function actualizarRegistro(id, { horaEntrada, horaSalida, observaciones }) {
  const { data, error } = await supabase
    .from("registros_horario")
    .update({
      hora_entrada: normalizarHora(horaEntrada),
      hora_salida: normalizarHora(horaSalida),
