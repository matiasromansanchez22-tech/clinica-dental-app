...
// Cobros de "Instalación (contado)" / "Instalación (2 cuotas)" de un
// paciente puntual — para saber, al abrir su cobro, si ya pagó la 1ª
// cuota (y así sugerir el valor de la 2ª) o si ya está todo pagado.
export async function obtenerCobrosInstalacion(pacienteId) {
  const { data, error } = await supabase
    .from("caja_ortodoncia")
    .select("concepto, importe, fecha")
    .eq("paciente_id", pacienteId)
    .in("concepto", ["Instalación (contado)", "Instalación (2 cuotas)"])
    .order("fecha", { ascending: true });
  if (error) throw error;
  return data || [];
}

// Lo mismo pero de TODOS los pacientes de una — para la columna "Estado de
// instalación" de la lista de pacientes, sin tener que consultar uno por
// uno. Devuelve un mapa paciente_id -> lista de cobros de instalación.
export async function obtenerCobrosInstalacionTodos() {
  const { data, error } = await supabase
    .from("caja_ortodoncia")
    .select("paciente_id, concepto, importe, fecha")
    .in("concepto", ["Instalación (contado)", "Instalación (2 cuotas)"])
    .order("fecha", { ascending: true });
  if (error) throw error;
  const porPaciente = {};
  for (const c of data || []) {
    if (!porPaciente[c.paciente_id]) porPaciente[c.paciente_id] = [];
    porPaciente[c.paciente_id].push(c);
  }
  return porPaciente;
}
