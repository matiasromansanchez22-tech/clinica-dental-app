...
export const CATEGORIAS_NOMENCLADOR = ["Común", "Prótesis"];

export async function obtenerObrasSociales() {
  const { data, error } = await supabase
    .from("obras_sociales")
    .select("id, nombre, cubre_fija_protesis")
    .order("nombre");
...
export async function obtenerNomencladorPorObraSocial(obraSocial, busqueda) {
  let query = supabase
    .from("nomenclador")
    .select(
      "id, obra_social, codigo, prestacion_os, valor_os, id_catalogo, prestacion_interna, copago_oficial, estado, categoria, catalogo:catalogo_prestaciones(valor_efectivo)"
    )
...
export async function recalcularCopagosSobreParticular() {
  ...
  let actualizadas = 0;
  let omitidas = 0;
  const LOTE = 40; // varias actualizaciones en paralelo por tanda, en vez de una por una
  while (true) {
    ...
    for (const fila of filas) {
      ...
      pendientes.push({ id: fila.id, nuevoCopago: Math.round((valorEfectivo * porcentaje) / 100) });
    }

    for (let i = 0; i < pendientes.length; i += LOTE) {
      const tanda = pendientes.slice(i, i + LOTE);
      const resultados = await Promise.all(
        tanda.map((p) => supabase.from("nomenclador").update({ copago_oficial: p.nuevoCopago }).eq("id", p.id))
      );
      const errorTanda = resultados.find((r) => r.error);
      if (errorTanda) throw errorTanda.error;
      actualizadas += tanda.length;
    }
...
export async function eliminarExcepcionCopago(id) {
  await moverAPapelera("configuracion_copago_excepcion", id);
}
