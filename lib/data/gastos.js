...
export async function crearGasto(datos) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // Solo se mandan si vienen con un valor: así no rompe la carga de
      // gastos comunes en clínicas donde todavía no se corrieron las
      // migraciones que agregan estas columnas (mecanico, comprobante_path).
      ...(datos.mecanico ? { mecanico: datos.mecanico } : {}),
      ...(datos.comprobantePath ? { comprobante_path: datos.comprobantePath } : {}),
    })
    .select()
    .single();
  if (error) throw error;
...
// Crea el gasto y, si la categoría está marcada "sale de la reserva"
// (alquiler, impuestos, proveedores, etc.), además descuenta ese mismo
// monto de la reserva de Consultorio — igual que hace GastoFormModal al
// cargar uno a mano. Se usa también desde Gastos Recurrentes, para no
// duplicar esta lógica en dos lugares.
export async function crearGastoConReserva(datos, categorias) {
  const nuevoGasto = await crearGasto(datos);
  const categoriaElegida = categorias.find((c) => c.nombre === datos.categoria);
  if (categoriaElegida?.sale_de_reserva) {
    const cuenta = datos.medioPago === "Efectivo" ? "Efectivo" : "Banco";
