...
import { enviarAviso } from "@/lib/data/avisos";
...
function diasDesde(fechaISO, hoyISO) {
  ...
  return Math.round((new Date(a2, m2 - 1, d2) - new Date(a1, m1 - 1, d1)) / (1000 * 60 * 60 * 24));
}

// Seguimiento de ventas: presupuestos que siguen en "Pendiente" y ya
// llevan varios días sin que el paciente responda — para llamarlo antes
// de perder la venta, en vez de dejar que se pierda en silencio.
export async function obtenerPresupuestosSinRespuesta(diasMinimo = 5) {
  ...
  const pendientes = data
    .map(mapearFilaPresupuesto)
    .map((p) => ({ ...p, diasSinRespuesta: diasDesde(p.fecha, hoy) }))
    .filter((p) => p.diasSinRespuesta >= diasMinimo)
    .sort((a, b) => b.diasSinRespuesta - a.diasSinRespuesta);

  if (pendientes.length === 0) return pendientes;

  const { data: seguimientos, error: errorSeg } = await supabase
    .from("presupuestos_seguimientos")
    .select("presupuesto_id, fecha_envio")
    .in(
      "presupuesto_id",
  ...
  const ultimoEnvioPorPresupuesto = new Map();
  for (const s of seguimientos) {
    if (!ultimoEnvioPorPresupuesto.has(s.presupuesto_id)) {
      ultimoEnvioPorPresupuesto.set(s.presupuesto_id, s.fecha_envio);
    }
  }

  return pendientes.map((p) => ({
    ...p,
    ultimoSeguimientoEnviado: ultimoEnvioPorPresupuesto.get(p.id) || null,
  }));
}

// Para el puntito rojo del menú.
export async function obtenerCantidadPresupuestosSinRespuesta() {
  const lista = await obtenerPresupuestosSinRespuesta();
  return lista.length;
}

// Registra que se mandó (o se volvió a mandar) el mensaje de seguimiento
// de este presupuesto — para llevar el control de a quién ya se le
// escribió y cuándo.
export async function registrarSeguimientoEnviado(presupuestoId) {
  const {
    data: { user },
  } = await supabase.auth.getUser();
  const { error } = await supabase.from("presupuestos_seguimientos").insert({
...
export async function obtenerPlanesFinanciacion() {
  const { data, error } = await supabase
    .from("planes_financiacion")
    .select(
      `id, presupuesto_id, numero_plan, numero_presupuesto, fecha, total_tratamiento, anticipo_acordado, anticipo_cobrado,
...
export async function obtenerHistorialPagosPlan(plan) {
  const [{ data: pagosCaja, error: e1 }, { data: pagosHistoricos, error: e2 }] = await Promise.all([
    supabase
      .from("caja_general")
      .select("id, fecha, pago, medio_pago, observaciones, prestaciones_realizadas")
  ...
  const deCaja = (pagosCaja || []).map((p) => ({
    ...
    prestacionesRealizadas: p.prestaciones_realizadas || [],
    origen: "Caja",
  }));
