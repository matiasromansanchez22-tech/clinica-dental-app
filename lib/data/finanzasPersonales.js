...
// El panel Consultorio es compartido entre los dos dueños. El panel
// Personal es privado de cada uno — la base de datos permite que
// cualquier dueño escriba ahí (hace falta, por ej. para poder cargar un
// sueldo "para" el otro), pero acá filtramos para que cada uno solo VEA
// lo suyo en la pantalla.
...
export async function obtenerMovimientosPersonales({ fechaInicio, fechaFin, panel }) {
  ...
  if (panel === "Personal") query = query.eq("usuario_id", await idUsuarioActual());
...
export async function obtenerSaldosPersonales(panel) {
  let query = supabase.from("movimientos_personales").select("cuenta, tipo, monto").eq("panel", panel);
  if (panel === "Personal") query = query.eq("usuario_id", await idUsuarioActual());
  const { data, error } = await query;
  if (error) throw error;
