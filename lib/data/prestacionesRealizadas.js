...
export async function obtenerPendienteCobroOrtodoncia(pacienteOrtodonciaId) {
  const { data, error } = await supabase
    .from("prestaciones_realizadas_agenda")
    .select("id, prestacion, bracket_reposicion, cantidad_brackets, created_at")
...
export async function obtenerAvisosPendientesDeCobro() {
  const { data, error } = await supabase
    .from("prestaciones_realizadas_agenda")
    .select("paciente_id, paciente_ortodoncia_id, profesional_id, prestacion, bracket_reposicion, cantidad_brackets, created_at")
    .eq("cobrado", false)
  ...
  for (const r of data) {
    ...
    const etiqueta = r.bracket_reposicion
