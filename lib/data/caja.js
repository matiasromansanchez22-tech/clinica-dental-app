...
// Nombres de las prestaciones del presupuesto original de un plan — para
// poder tildar, al cobrar una cuota, cuál de ellas se hizo ese día (dato
// solo informativo, no participa de la liquidación).
export async function obtenerPrestacionesDelPresupuesto(presupuestoId) {
  if (!presupuestoId) return [];
  const { data, error } = await supabase.from("presupuestos").select("prestaciones").eq("id", presupuestoId).single();
  if (error) throw error;
  return (data.prestaciones || []).map((p) => p.prestacion).filter(Boolean);
}
...
export async function obtenerPrestacionesParticular() {
  const { data, error } = await supabase
    ...
    .select("id, prestacion, categoria, valor_lista, valor_efectivo, especialidad")
    .eq("estado", "Activo")
    .eq("particular", true)
    .order("prestacion");
  if (error) throw error;
...
export async function obtenerPrestacionesObraSocial(obraSocial) {
  const { data, error } = await supabase
    .from("nomenclador")
    .select("id, codigo, prestacion_os, prestacion_interna, id_catalogo, copago_oficial, valor_os, categoria")
...
const CAMPOS_CAJA_GENERAL = `id, fecha, tipo, cobertura, modalidad, numero_cuota, prestaciones, prestaciones_realizadas, importe_total, pago, medio_pago,
...
export async function obtenerCobrosPorFecha(fecha) {
  ...
  return data.map((f) => ({
    ...
    prestacionesRealizadas: f.prestaciones_realizadas || [],
...
// devuelve cuántos quedan pendientes.
export async function obtenerCantidadCobrosConSaldoPendiente() {
  const cobros = await obtenerCobrosConSaldoPendiente();
...
export async function crearCobro(datos) {
  const { data: caja, error } = await supabase
    ...
    .insert({
      ...
      prestaciones_realizadas: datos.prestacionesRealizadas?.length ? datos.prestacionesRealizadas : null,
    })
    .select()
    .single();
  if (error) throw error;
...
export async function actualizarPlanDesdeCaja(numeroPlan) {
  ...
  // El último pago puede haber sido uno real de Caja o uno histórico
  // cargado a mano — se compara por fecha entre los dos para no perder de
  // vista el más reciente de verdad.
  const todosLosPagos = [
    ...pagos.map((p) => ({ fecha: p.fecha, medioPago: p.medio_pago })),
    ...pagosHistoricos.map((p) => ({ fecha: p.fecha, medioPago: null })),
  ];
  const ultimoPago = todosLosPagos.sort((a, b) => (a.fecha < b.fecha ? 1 : a.fecha > b.fecha ? -1 : 0))[0];
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      medio_pago_ultimo_pago: ultimoPago?.medioPago || null,
