  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, fecha, tipo, cobertura, modalidad, numero_cuota, prestaciones, importe_total, pago, medio_pago,
       estado_cobro, observaciones, id_documento, usuario_id, cerrado, created_at,
...
export async function actualizarPlanDesdeCaja(numeroPlan) {
  ...
  const totalPagado = pagos.reduce((acc, p) => acc + Number(p.pago), 0);
...
export async function eliminarCobro(cobro) {
  const { error: errorFactura } = await supabase.from("facturacion_obras_sociales").delete().eq("caja_id", cobro.id);
  if (errorFactura) throw errorFactura;

  const { error } = await supabase.from("caja_general").delete().eq("id", cobro.id);
  if (error) throw error;
