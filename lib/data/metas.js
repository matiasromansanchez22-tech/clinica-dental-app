...
// Meta de consultas por profesional (circuito de ventas) — cuántas
// consultas nuevas se le asignan a cada uno ese mes.
export async function obtenerMetasConsultasMes(anio, mes) {
  const { data, error } = await supabase
    .from("metas_consultas_profesional")
    .select("profesional_id, consultas_objetivo")
    .eq("anio", anio)
    .eq("mes", mes);
  if (error) throw error;
  return Object.fromEntries(data.map((f) => [f.profesional_id, f.consultas_objetivo]));
}

export async function guardarMetaConsultas(profesionalId, anio, mes, consultasObjetivo) {
  const { error } = await supabase.from("metas_consultas_profesional").upsert(
    {
      profesional_id: profesionalId,
      anio,
      mes,
      consultas_objetivo: Number(consultasObjetivo) || 0,
      updated_at: new Date().toISOString(),
    },
    { onConflict: "profesional_id,anio,mes" }
  );
  if (error) throw error;
}
