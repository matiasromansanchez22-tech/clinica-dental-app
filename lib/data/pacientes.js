...
// cargado en Caja — es decir, ya arrancaron un tratamiento de verdad (no
// se quedaron solo en la consulta). A diferencia de Ortodoncia (donde
// alcanza con el tipo de turno), acá esa columna no se carga con la misma
// disciplina, así que se usa esta señal más confiable: plata de por medio
// o un presupuesto que el paciente aceptó.
export async function obtenerIdsPacientesConActividad() {
  const [{ data: presupuestos, error: e1 }, { data: cobros, error: e2 }] = await Promise.all([
    ...
    supabase.from("caja_general").select("paciente_id"),
  ]);
  if (e1) throw e1;
  if (e2) throw e2;
  ...
  for (const c of cobros) if (c.paciente_id) idsConActividad.add(c.paciente_id);
