...
// tratamiento de verdad. A diferencia de Ortodoncia (donde alcanza con el
// tipo de turno), acá esa columna no se carga con la misma disciplina, así
// que se usa esta señal más confiable: plata de por medio o un
// presupuesto que el paciente aceptó.
export async function obtenerIdsPacientesSoloConsulta() {
  ...
  const { data: pacientes, error: e3 } = await supabase.from("pacientes").select("id");
  if (e3) throw e3;
