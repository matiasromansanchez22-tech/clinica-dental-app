...
// IDs de pacientes que ya tuvieron un presupuesto aceptado o un cobro
// cargado en Caja que representa más que la consulta inicial — es decir,
// ya arrancaron un tratamiento de verdad (no se quedaron solo en la
// consulta). A diferencia de Ortodoncia (donde alcanza con el tipo de
// turno), acá esa columna no se carga con la misma disciplina, así que se
// usa esta señal más confiable: plata de por medio (más allá de la
// consulta) o un presupuesto que el paciente aceptó.
export async function obtenerIdsPacientesConActividad() {
  const [{ data: presupuestos, error: e1 }, { data: cobros, error: e2 }] = await Promise.all([
    supabase.from("presupuestos").select("paciente_id").eq("estado", "Aceptado"),
    supabase.from("caja_general").select("paciente_id, prestaciones"),
  ]);
  if (e1) throw e1;
  if (e2) throw e2;

  const idsConActividad = new Set();
  for (const p of presupuestos) if (p.paciente_id) idsConActividad.add(p.paciente_id);
  for (const c of cobros) {
    if (c.paciente_id && !esCobroSoloConsulta(c.prestaciones)) idsConActividad.add(c.paciente_id);
  }
  return idsConActividad;
}

// Pacientes que nunca tuvieron un presupuesto aceptado ni un cobro
// cargado en Caja — todavía "solo consultaron", no arrancaron ningún
// tratamiento de verdad.
export async function obtenerIdsPacientesSoloConsulta() {
  const idsConActividad = await obtenerIdsPacientesConActividad();

  const { data: pacientes, error } = await supabase.from("pacientes").select("id");
  if (error) throw error;

  return new Set(pacientes.filter((p) => !idsConActividad.has(p.id)).map((p) => p.id));
}

const ORDEN_ESTADO_VENTA = {
  "No aceptó presupuesto": 0,
  "Presupuesto pendiente": 1,
  "En tratamiento": 2,
  "Tratamiento terminado": 3,
};

// Estado de venta de cada paciente en el circuito — calculado solo, no se
// carga a mano en ningún lado. Se arma a partir de sus presupuestos, su
// plan de financiación y sus cobros reales, y se queda con el paso más
// avanzado si hay señales mezcladas (ej. un presupuesto viejo Anulado y
// uno nuevo Pendiente → se queda con "Presupuesto pendiente").
export async function obtenerEstadosVentaPacientes() {
  const [{ data: presupuestos, error: e1 }, { data: planes, error: e2 }, { data: cobros, error: e3 }] =
    await Promise.all([
      supabase.from("presupuestos").select("paciente_id, estado"),
      supabase.from("planes_financiacion").select("paciente_id, estado_plan"),
      supabase.from("caja_general").select("paciente_id, prestaciones"),
    ]);
  if (e1) throw e1;
  if (e2) throw e2;
  ...
  function marcar(id, estado) {
    ...
    if (!actual || ORDEN_ESTADO_VENTA[estado] > ORDEN_ESTADO_VENTA[actual]) estados.set(id, estado);
  }

  for (const p of presupuestos) {
    if (p.estado === "Pendiente") marcar(p.paciente_id, "Presupuesto pendiente");
    else if (p.estado === "Anulado") marcar(p.paciente_id, "No aceptó presupuesto");
    else if (p.estado === "Aceptado") marcar(p.paciente_id, "En tratamiento");
  }
  for (const pl of planes) {
    if (pl.estado_plan === "Finalizado") marcar(pl.paciente_id, "Tratamiento terminado");
    else if (pl.estado_plan === "Activo") marcar(pl.paciente_id, "En tratamiento");
  }
  for (const c of cobros) {
    if (c.paciente_id && !esCobroSoloConsulta(c.prestaciones)) marcar(c.paciente_id, "En tratamiento");
  }
...
// Plan de tratamiento: un solo texto por paciente (se edita y reemplaza,
// no queda historial de versiones) — separado a propósito del Historial
// clínico, que sí es un registro fechado visita a visita.
export async function obtenerPlanTratamiento(pacienteId) {
  const { data, error } = await supabase.from("pacientes").select("plan_tratamiento").eq("id", pacienteId).single();
  if (error) throw error;
  return data.plan_tratamiento;
}

export async function actualizarPlanTratamiento(pacienteId, texto) {
  const { error } = await supabase
    .from("pacientes")
    .update({ plan_tratamiento: texto?.trim() || null })
    .eq("id", pacienteId);
  if (error) throw error;
}
