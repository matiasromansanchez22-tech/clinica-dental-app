...
import { obtenerIdCatalogoMecanico } from "@/lib/data/mecanicosPrecios";
...
function mapearTrabajo(f) {
  return {
    ...
    catalogo: f.catalogo
      ? {
          prestacion: f.catalogo.prestacion,
          valorLista: Number(f.catalogo.valor_lista) || 0,
          valorEfectivo: Number(f.catalogo.valor_efectivo) || 0,
        }
      : null,
  };
}
...
const SELECT_TRABAJO = `id, tipo_paciente, paciente_id, paciente_nombre, profesional_id, tipo_trabajo, pieza,
  laboratorio, estado, fecha_inicio, fecha_ultimo_evento, fecha_alta, observaciones, valor, id_catalogo,
  profesional:profesionales(nombre),
  catalogo:catalogo_prestaciones(prestacion, valor_lista, valor_efectivo)`;

// Busca en el catálogo una prestación cuyo nombre coincida exactamente
// (sin importar mayúsculas/espacios) con el tipo de trabajo escrito — así
// el trabajo queda vinculado solo, sin que nadie tenga que elegirlo a
// mano, siempre que el nombre coincida con el catálogo.
export function buscarIdCatalogoPorNombre(tipoTrabajo, catalogo) {
  const normalizado = (tipoTrabajo || "").trim().toLowerCase();
  if (!normalizado) return null;
  const encontrado = catalogo.find((c) => c.prestacion.trim().toLowerCase() === normalizado);
  return encontrado?.id || null;
}

// El mecánico no siempre le pone a un trabajo el mismo nombre que tiene esa
// prestación en nuestro catálogo (ej. mecánico dice "Corona Zirconio
// Layer", nosotros "Corona de Zirconio") — comparar nombres a ciegas generaba
// trabajos sin vincular o vinculados a lo que no era. Por eso primero se
// busca el vínculo que ya se cargó a mano en la comparativa de mecánicos
// (para ese laboratorio + ese nombre exacto) y, si no hay laboratorio
// cargado o todavía no se vinculó ese trabajo ahí, recién como respaldo se
// compara el nombre directamente contra el catálogo.
export async function resolverIdCatalogo(laboratorio, tipoTrabajo, catalogo) {
  if (laboratorio?.trim()) {
    try {
      const idDesdeMecanico = await obtenerIdCatalogoMecanico(laboratorio, tipoTrabajo);
      if (idDesdeMecanico) return idDesdeMecanico;
    } catch {
      // Si todavía no existe el vínculo en la base (o falla la consulta),
      // no debe bloquear el guardado del trabajo — se sigue con el
      // respaldo de comparar por nombre.
    }
  }
  return buscarIdCatalogoPorNombre(tipoTrabajo, catalogo);
}
...
// Al marcar el envío es cuando realmente se sabe (o se confirma) a qué
// mecánico va — por eso este es el momento de guardar/corregir ese dato,
// no solo el estado.
export async function marcarTrabajoEnviado(trabajoId, fecha, laboratorio) {
...
// Corrige datos del trabajo por si se cargó algo mal (tipo de trabajo,
// pieza, laboratorio, profesional). No toca estado, fechas ni eventos —
// para eso están agregarEventoTrabajo/marcarTrabajoEnviado.
export async function actualizarTrabajoLaboratorio(id, datos) {
  const { data, error } = await supabase
    .from("laboratorio_trabajos")
    .update({
      tipo_trabajo: datos.tipoTrabajo,
...
// notaCalculo (opcional): si el valor salió de la calculadora por piezas,
// deja registrado el detalle del cálculo en observaciones para poder
// auditarlo después (ej. por qué un trabajo quedó en $166.600).
export async function actualizarValorTrabajo(id, valor, notaCalculo) {
  const cambios = { valor: valor === "" || valor === null ? null : Number(valor), updated_at: new Date().toISOString() };
  if (notaCalculo) {
    ...
    cambios.observaciones = actual.observaciones ? `${actual.observaciones}\n${notaCalculo}` : notaCalculo;
  }
  const { error } = await supabase.from("laboratorio_trabajos").update(cambios).eq("id", id);
  if (error) throw error;
}
