...
function mapearPedido(f) {
  return {
    ...
    items: f.items || [],
    subtotal: Number(f.subtotal ?? f.total),
...
export async function crearPedido(datos) {
  const subtotal = calcularTotalItems(datos.items);
  const descuentoPorcentaje = Number(datos.descuentoPorcentaje) || 0;
...
export async function leerFacturaPedidoConIA(archivo, _categoriasNoUsadas) {
  ...
  const res = await fetch("/api/leer-factura-pedido", {
    method: "POST",
