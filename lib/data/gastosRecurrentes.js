...
export async function obtenerPagosDelMesPorNombre(fechaInicio, fechaFin) {
  ...
  for (const g of data) {
    ...
    porNombre[g.descripcion].push({ fecha: g.fecha, monto: Number(g.monto) });
  }
  return porNombre;
}
...
export async function registrarPagoRecurrente(gastoRecurrente, { fecha, monto, medioPago }, categorias) {
