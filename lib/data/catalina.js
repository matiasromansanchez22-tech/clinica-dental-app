...
const CAMPOS_LISTA_CATALINA = "id, nombre, categoria, cantidad, unidad, notas, created_at, updated_at";

export async function obtenerListaCompraCatalina() {
  const { data, error } = await supabase.from("catalina_lista_compras").select(CAMPOS_LISTA_CATALINA).order("nombre");
  if (error) throw error;
  return data;
}

export async function agregarItemListaCompraCatalina(datos) {
  const { data, error } = await supabase
    .from("catalina_lista_compras")
    .insert({
      nombre: datos.nombre,
      categoria: datos.categoria,
    ...
    .select(CAMPOS_LISTA_CATALINA)
    .single();
  if (error) throw error;
  return data;
}

export async function actualizarItemListaCompraCatalina(id, datos) {
  const { data, error } = await supabase
    .from("catalina_lista_compras")
    .update({
      nombre: datos.nombre,
...
export async function eliminarItemListaCompraCatalina(id) {
  await moverAPapelera("catalina_lista_compras", id);
}

// Al tildar un producto de la lista: suma la cantidad al stock (mismo
// criterio que registrarCompraCatalina) y lo saca de la lista.
export async function marcarCompradoListaCatalina(item) {
  await registrarCompraCatalina([
    { nombre: item.nombre, categoria: item.categoria, cantidad: item.cantidad, unidad: item.unidad },
  ]);
  await moverAPapelera("catalina_lista_compras", item.id);
}
