...
// correr la migración 083/097), sigue funcionando el Balance Mensual normal
// en vez de romperse.
async function obtenerCobrosGeneralParaBalance(fechaInicio, fechaFin) {
  let resultado = await supabase
    .from("caja_general")
    .select("fecha, pago, medio_pago, desglose_pago, tipo")
    .gte("fecha", fechaInicio)
    .lte("fecha", fechaFin);
  if (resultado.error?.message?.includes("desglose_pago")) {
    resultado = await supabase
      .from("caja_general")
      .select("fecha, pago, medio_pago, tipo")
      .gte("fecha", fechaInicio)
      .lte("fecha", fechaFin);
  }
  return resultado;
...
async function obtenerCobrosOrtoParaBalance(fechaInicio, fechaFin) {
  let resultado = await supabase
    .from("caja_ortodoncia")
    .select("fecha, importe, medio_pago, desglose_pago")
    .gte("fecha", fechaInicio)
    .lte("fecha", fechaFin);
  if (resultado.error?.message?.includes("desglose_pago")) {
    resultado = await supabase
      .from("caja_ortodoncia")
      .select("fecha, importe, medio_pago")
      .gte("fecha", fechaInicio)
      .lte("fecha", fechaFin);
  }
  return resultado;
...
export async function obtenerBalanceMensual(fechaInicio, fechaFin) {
  const [
    ...
    { data: pagosAsor, error: errorAsor },
  ] = await Promise.all([
    obtenerCobrosGeneralParaBalance(fechaInicio, fechaFin),
    obtenerCobrosOrtoParaBalance(fechaInicio, fechaFin),
    ...
    supabase
      ...
      .gte("fecha", fechaInicio)
      .lte("fecha", fechaFin),
    supabase.from("pagos_asor").select("fecha, monto").gte("fecha", fechaInicio).lte("fecha", fechaFin),
  ]);
  if (errorGeneral) throw errorGeneral;
  if (errorOrto) throw errorOrto;
  if (errorGastos) throw errorGastos;
  if (errorPagos) throw errorPagos;
  if (errorAsor) throw errorAsor;
  ...
  const ingresosAsor = (pagosAsor || []).reduce((acc, p) => acc + Number(p.monto), 0);
  const ingresosTotal = ingresosGeneral + ingresosOrtodoncia + ingresosAsor;
  ...
  const ingresosPorEspecialidad = [
    ...
    { clave: "Reembolsos ASOR (obras sociales)", monto: ingresosAsor },
  ].filter((f) => f.monto > 0);

  // Un cobro con pago mixto reparte su desglose real entre medios, en vez
  // de sumar todo a uno solo.
  const ingresosGeneralPorMedio = (cobrosGeneral || []).flatMap((c) =>
    c.desglose_pago?.length
      ? c.desglose_pago.map((p) => ({ medio_pago: p.medio, monto: Number(p.monto) }))
      : [{ medio_pago: c.medio_pago, monto: Number(c.pago) }]
  );
  const ingresosOrtoPorMedio = (cobrosOrto || []).flatMap((c) =>
    c.desglose_pago?.length
      ? c.desglose_pago.map((p) => ({ medio_pago: p.medio, monto: Number(p.monto) }))
      : [{ medio_pago: c.medio_pago, monto: Number(c.importe) }]
  );
  const todosLosIngresos = [
    ...ingresosGeneralPorMedio,
    ...ingresosOrtoPorMedio,
    ...(pagosAsor || []).map((p) => ({ medio_pago: "Transferencia", monto: Number(p.monto) })),
  ];
...
// Plata acumulada desde que arrancó la clínica (todos los ingresos menos
// todos los egresos, sin filtro de fecha). Se usa en el Panel de
// Estadísticas para el indicador de "plata suficiente para invertir".
export async function obtenerBalanceAcumuladoTotal() {
  const [
    { data: cobrosGeneral, error: errorGeneral },
    { data: cobrosOrto, error: errorOrto },
    { data: gastos, error: errorGastos },
    ...
    supabase.from("caja_general").select("pago"),
    supabase.from("caja_ortodoncia").select("importe"),
    supabase.from("gastos").select("monto"),
    supabase.from("pagos_profesionales").select("monto"),
    supabase.from("pagos_asor").select("monto"),
  ]);
  if (errorGeneral) throw errorGeneral;
  if (errorOrto) throw errorOrto;
  if (errorGastos) throw errorGastos;
  ...
  const ingresosTotal =
    (cobrosGeneral || []).reduce((acc, c) => acc + Number(c.pago), 0) +
    (cobrosOrto || []).reduce((acc, c) => acc + Number(c.importe), 0) +
    (pagosAsor || []).reduce((acc, p) => acc + Number(p.monto), 0);
  const egresosTotal =
    (gastos || []).reduce((acc, g) => acc + Number(g.monto), 0) +
    (pagosProfesionales || []).reduce((acc, p) => acc + Number(p.monto), 0);

  return { ingresosTotal, egresosTotal, acumulado: ingresosTotal - egresosTotal };
}
