...
export async function obtenerAutorizacionesFotosOrtodoncia() {
  try {
    const { data, error } = await supabase.from("pacientes_ortodoncia").select("id, autoriza_fotos_redes");
    if (error) throw error;
...
export async function actualizarAutorizacionFotosOrtodoncia(id, autoriza) {
  const { error } = await supabase.from("pacientes_ortodoncia").update({ autoriza_fotos_redes: autoriza }).eq("id", id);
  if (error) throw error;
}
...
// controles cargados (la base lo protege sola con esa restricción) —
...
export async function eliminarPacienteOrtodoncia(id) {
  try {
    await moverAPapelera("pacientes_ortodoncia", id);
  } catch (e) {
    if (e.code === "23503") {
      throw new Error(
        "No se puede borrar: este paciente ya tiene turnos, cobros o controles cargados. Si fue un error de carga, marcalo como Inactivo en vez de borrarlo."
      );
    }
    throw e;
  }
...
// Cuando un paciente que estaba en "Consulta" arranca el tratamiento de
// verdad (se le cargó un turno de instalación, o alguien lo marca a mano),
// pasa solo a "Activo" — no hace falta cargarlo de nuevo, ya tiene su
// nombre/WhatsApp/ortodoncista habitual desde que se agendó la consulta.
export async function marcarInicioTratamiento(id, fechaInstalacion) {
  const { data, error } = await supabase
    .from("pacientes_ortodoncia")
    .update({
...
// Mismo criterio de "nombres parecidos" de arriba, pero contra una lista
// que ya se tiene en memoria (ej. al crear un turno nuevo, donde ya se
// cargaron todos los pacientes) — sin pegarle de nuevo a la base. Sirve
// para avisar de un posible duplicado ahí también: si no se escribe el
// nombre exactamente igual (un acento, un espacio de más), el paciente
// existente no se reconoce solo y se termina creando uno nuevo sin
// querer — sobre todo con los que recién están "en consulta", que todavía
// no tienen turnos ni cobros que hagan notar el duplicado.
export function buscarParecidoEnLista(nombre, pacientes, idExcluido) {
  const texto = nombre.trim();
  if (texto.length < 3) return [];
  return pacientes
    .filter((p) => p.id !== idExcluido)
...
export async function actualizarConfiguracionOrtodonciaBulk(pares) {
  const filas = Object.entries(pares).map(([clave, valor]) => ({ clave, valor: Number(valor) }));
  const { error } = await supabase.from("configuracion_ortodoncia").upsert(filas, { onConflict: "clave" });
  if (error) throw error;
}
