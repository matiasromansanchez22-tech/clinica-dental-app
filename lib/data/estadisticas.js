...
// La fecha del primer turno de cada paciente (no cancelado ni reprogramado,
// y ya pasado) — a diferencia de la fecha de alta del paciente, esto
// refleja cuándo vino de verdad por primera vez a consultarnos, sin
// importar si el registro del paciente ya existía de antes (por ejemplo,
// los que se cargaron en la migración inicial a la app).
...
export async function obtenerContextoConversion() {
  const [primerTurnoGeneral, primerTurnoOrtodoncia, idsConActividad, estadoOrtodoncia] = await Promise.all([
    obtenerPrimerTurnoPorPaciente("turnos_general"),
    obtenerPrimerTurnoPorPaciente("turnos_ortodoncia"),
...
export async function obtenerResumenMensual(anio, mes, contexto) {
  ...
  const [historialesMarcados, consentimientosMarcados, balance] = await Promise.all([
    contarPorFecha("pacientes", "historia_clinica_marcada_en", desde, hasta),
    contarPorFecha("pacientes", "consentimiento_marcado_en", desde, hasta),
