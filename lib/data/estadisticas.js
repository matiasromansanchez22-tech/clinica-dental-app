...
// La fecha de alta del paciente de Odontología General — en la práctica es
// la fecha real de su primera consulta, porque la ficha se crea en el
// momento de agendarle el primer turno (no se recicla una ficha vieja para
// un paciente nuevo). Esto deja afuera de forma automática a todos los que
// ya estaban cargados de antes (la migración inicial a la app u otros
// pacientes viejos), sin depender de ningún campo de "tipo de atención"
// (que no es confiable: queda "Consulta" por defecto en casi todos los
// turnos, se cargue lo que se cargue).
async function obtenerFechaAltaPacientes(tabla) {
  const { data, error } = await supabase.from(tabla).select("id, created_at");
  if (error) throw error;
  return new Map(data.map((p) => [p.id, p.created_at.slice(0, 10)]));
}
...
export async function obtenerContextoConversion() {
  const [altaGeneral, primerTurnoOrtodoncia, idsConActividad, estadoOrtodoncia] = await Promise.all([
    obtenerFechaAltaPacientes("pacientes"),
    obtenerPrimeraConsultaOrtodonciaPorPaciente(),
    obtenerIdsPacientesConActividad(),
    obtenerEstadoPacientesOrtodoncia(),
  ]);

  return { altaGeneral, primerTurnoOrtodoncia, idsConActividad, estadoOrtodoncia };
}

// De los pacientes dados de alta en el mes (fecha real de su primera
// consulta, ver obtenerFechaAltaPacientes), cuántos ya arrancaron un
// tratamiento de verdad (no importa si fue ese mismo mes o más adelante) —
// se apoya en la misma señal que "En consulta" (presupuesto aceptado o
// cobro en Caja).
function contarConversionGeneral(fechaInicio, fechaFin, altaGeneral, idsConActividad) {
  let nuevos = 0;
  let comenzaronTratamiento = 0;
  for (const [id, fecha] of altaGeneral) {
...
export async function obtenerResumenMensual(anio, mes, contexto) {
  ...
  const conversionGeneral = contarConversionGeneral(fechaInicio, fechaFin, ctx.altaGeneral, ctx.idsConActividad);
