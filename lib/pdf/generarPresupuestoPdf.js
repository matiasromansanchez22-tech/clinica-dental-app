...
const AMBAR_CLARO = [255, 251, 235];
const AMBAR_OSCURO = [146, 100, 6];
...
export async function generarPresupuestoPdf(presupuesto, vigenciaHasta, comparacionPrecios, comparacionPrioridad) {
  ...
  autoTable(doc, {
    ...
    body: presupuesto.prestaciones.map((p) => [
      `${p.prioridad ? "⭐ " : ""}${p.prestacion}`,
      String(p.cantidad),
      p.tipoPrecio,
      formatoPesos(p.importe),
    ]),
    theme: "plain",
  ...
  if (comparacionPrecios) {
    const ahorra = comparacionPrecios.totalLista > comparacionPrecios.totalEfectivo;
    const altoBloque = ahorra ? 22 : 16;
    doc.setFillColor(...TAN_CLARO);
    doc.roundedRect(margen, y, anchoUtil, altoBloque, 2, 2, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    doc.setTextColor(...MARRON);
    doc.text("SEGÚN CÓMO PREFIERA PAGAR", margen + 4, y + 6);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.setTextColor(...GRIS_TEXTO);
    doc.text(`Valor de lista: ${formatoPesos(comparacionPrecios.totalLista)}`, margen + 4, y + 12);
    doc.text(`Valor en efectivo: ${formatoPesos(comparacionPrecios.totalEfectivo)}`, margen + anchoUtil / 2, y + 12);
    if (ahorra) {
      doc.setFontSize(8.5);
      doc.setTextColor(...MARRON);
      doc.text(
        `Ahorra ${formatoPesos(comparacionPrecios.totalLista - comparacionPrecios.totalEfectivo)} pagando en efectivo`,
        margen + 4,
        y + 18
      );
    }
  ...
  if (comparacionPrioridad) {
    const ahorra = comparacionPrioridad.totalLista > comparacionPrioridad.totalEfectivo;
    const altoBloque = ahorra ? 22 : 16;
    doc.setFillColor(...AMBAR_CLARO);
    doc.roundedRect(margen, y, anchoUtil, altoBloque, 2, 2, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    doc.setTextColor(...AMBAR_OSCURO);
    doc.text("⭐ OPCIÓN MÁS ACCESIBLE — SOLO LO MARCADO COMO PRIORIDAD", margen + 4, y + 6);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.setTextColor(...GRIS_TEXTO);
    doc.text(`Valor de lista: ${formatoPesos(comparacionPrioridad.totalLista)}`, margen + 4, y + 12);
    doc.text(`Valor en efectivo: ${formatoPesos(comparacionPrioridad.totalEfectivo)}`, margen + anchoUtil / 2, y + 12);
    if (ahorra) {
      doc.setFontSize(8.5);
      doc.setTextColor(...AMBAR_OSCURO);
      doc.text(
        `Ahorra ${formatoPesos(comparacionPrioridad.totalLista - comparacionPrioridad.totalEfectivo)} pagando en efectivo`,
        margen + 4,
        y + 18
      );
    }
  ...
  if (presupuesto.modalidadPago) {
    const lineas = [`Modalidad: ${presupuesto.modalidadPago}`];
    if (presupuesto.modalidadPago === "Financiado") {
      const cuotas = Number(presupuesto.cantidadCuotas) || 0;
      const valorCuota = cuotas > 0 ? Math.round((Number(presupuesto.saldo) / cuotas) * 100) / 100 : 0;
      lineas.push(`Anticipo: ${formatoPesos(presupuesto.anticipo)}     Saldo a financiar: ${formatoPesos(presupuesto.saldo)}`);
      lineas.push(`${cuotas} cuotas de: ${formatoPesos(valorCuota)}`);
    }
