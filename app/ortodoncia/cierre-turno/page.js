...
import {
  calcularTotalesDelTurnoOrtodoncia,
  guardarCierreTurnoOrtodoncia,
  obtenerCierreTurnoOrtodoncia,
  obtenerCierresTurnoOrtodonciaDelDia,
  reabrirTurnoOrtodoncia,
} from "@/lib/data/cierresTurnoOrtodoncia";
...
export default function CierreTurnoOrtodonciaPage() {
  ...
  useEffect(() => {
    ...
    setMensaje(null);
    Promise.all([
      calcularTotalesDelTurnoOrtodoncia(fecha, user.id),
      obtenerCierreTurnoOrtodoncia(fecha, user.id),
  ...
  async function handleReabrir(c) {
    ...
    setError(null);
    try {
      await reabrirTurnoOrtodoncia(fecha, c.usuario_id);
      const delDia = await obtenerCierresTurnoOrtodonciaDelDia(fecha);
      setCierresDelDia(delDia);
      if (c.usuario_id === user.id) {
        setCierreExistente(null);
        const t = await calcularTotalesDelTurnoOrtodoncia(fecha, user.id);
        setTotales(t);
      }
    } catch (e) {
      setError(e.message);
  ...
  async function handleGuardar() {
    ...
    setMensaje(null);
    try {
      await guardarCierreTurnoOrtodoncia(fecha, user.id, perfil?.nombre || user.email, totales, observaciones);
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <h1 className="text-2xl font-bold text-gray-900">Cierre de Turno — Ortodoncia</h1>
      <p className="mt-1 text-sm text-gray-500">
        Suma solo los cobros que vos cargaste este día ({perfil?.nombre || user?.email}), separados por medio de
        pago — para que cada secretario/a cierre su propia parte de la caja de Ortodoncia.
      </p>
