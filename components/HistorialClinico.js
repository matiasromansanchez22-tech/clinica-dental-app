...
// Base genérica de "Historial clínico" — la usan HistorialClinicoOrtodoncia
// y HistorialClinicoGeneral, cada una pasándole sus propias funciones de
// datos (tablas distintas por sistema), para no duplicar la UI dos veces.
export default function HistorialClinico({
  ...
  async function cargar() {
    setCargando(true);
    try {
      setEntradas(await obtenerHistorial(pacienteId));
    } catch (e) {
      setError(e.message);
    } finally {
      setCargando(false);
  ...
  async function guardarNueva() {
    ...
    try {
      if (editando) {
        await actualizarEntrada(editando, nueva);
      } else {
        await crearEntrada({ pacienteId, ...nueva });
      }
